...
class Queue extends Loggable {
  constructor({
    ...
    me.workers.on("empty", () => {
      ...
      if (me.jobs.length) {
        me.warn(
          `All workers were destroyed, but there are ${me.jobs.length} items in the queue. Restarting.`
        );
    ...
    me.availableWorkers.on("add", () => {
      if (
        me.availableWorkers.size === me.workers.size &&
        me.jobs.length === 0
      ) {
        me.emit("stop");
      }
  ...
  async getWorker() {
    ...
    if (me.availableWorkers.isEmpty && me.workers.size < me.maxWorkers) {
      ...
      worker.on("log", ({ level, message, label, id }) =>
        me.log(level, message, label, id)
      );
  ...
  async queue({ requestId, items, config }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      me.info(
        `Added ${items.length} to the queue, current length is ${me.jobs.length}`
      );
      ...
      function onJobCancel() {
        ...
        reject(
          new RequestCancelError(
            `Request ${requestId} is cancelled by the client`
          )
        );
      }
  ...
  runJob(worker, job) {
    ...
    if (me.shouldContinueJob(job.requestId)) {
      ...
      worker
        ...
        .catch((e) => {
          ...
          if (++job.failCount <= MAX_FAILS) {
            worker.warn(
              `Job ${job.index + 1}/${job.length} for request ${
                job.requestId
              } failed, restarting`
            );
            ...
            worker.warn(
              `Job ${job.index + 1}/${job.length} for request ${
                job.requestId
              } failed after ${MAX_FAILS} attempts`
            );
      ...
      me.verbose(
        `Skipping job ${job.index + 1}/${job.length} for request ${
          job.requestId
        }`
      );
    }
  ...
  async next() {
    ...
    if (me.jobs.length) {
      ...
      if (me.shouldContinueJob(job.requestId)) {
        me.verbose(
          `Starting job ${job.index + 1}/${job.length} for request ${
            job.requestId
          }`
        );
...
class Worker extends Loggable {
    ...
    if (config.clientURL) {
      ...
      try {
        await page.goto(config.clientURL, {
          waitUntil: "load",
          referer: "bryntum_pdf_export_server",
        });
      } catch (e) {
        // Log warning, try to continue export
        me.warn(
          `Unable to open client url ${config.clientURL}.\nError: ${e.message}`
        );
      }
    } else {
  ...
  async processPageIntoPngBuffer(page, html, config) {
    ...
    // if content box starts at 0,0 then we can adjust content size perfectly
      ...
      const format = paperFormat[viewportConfig.format.toLowerCase()],
        width =
          viewportConfig.orientation === "portrait"
            ? format.width
            : format.height,
        height =
          viewportConfig.orientation === "portrait"
    ...
    me.verbose(
      `Taking screenshot of size ${viewportConfig.width}x${viewportConfig.height}`
    );
